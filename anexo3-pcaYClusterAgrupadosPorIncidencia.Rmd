...
```{r}
standardErrors <- gapSatistic$Tab[, "SE.sim"]
standardErrors
```

```{r}
# (Assuming you already have 'gap_stat' from running clusGap())

gap_values <- gapSatisticIncidents$Tab[, "gap"] # Extract gap statistic values
k_max <- which.max(gap_values) # Find k with max gap statistic

max_gap <- gap_values[k_max]
max_se <- gapSatisticIncidents$Tab[k_max, "SE.sim"]

# Apply the 1-SE Rule
optimal_k <- 13  # Initialize with max k
for (k in (k_max - 1):1) { # Iterate from k_max - 1 down to 1
  threshold <- max_gap - max_se  # Calculate threshold
...
print(paste("Optimal number of clusters (1-SE rule):", optimal_k))
```
...
sil2KI <- silhouette(kmeansModel2KI$cluster, diss_mat)

summary(sil2KI)
```

```{r}
fviz_silhouette(sil2KI)
```
...
kmeansModel4KI <- kmeans(dfGroupedByIncident, centers = 9, nstart = 25)
